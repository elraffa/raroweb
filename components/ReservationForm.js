...
const ReservationForm = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    if (error) {
      ...
      setMessage(
        `Hubo un error al hacer la reserva. Por favor intenta de nuevo. Si el problema persiste, contactanos a @raro_duo en IG.`
      );
    } else {
  ...
  return (
    <>
      <h5 className={styles.remainingPlaces}>
        {isLoading ? "Cargando..." : `Lugares disponibles: ${remainingPlaces}`}
      </h5>{" "}
